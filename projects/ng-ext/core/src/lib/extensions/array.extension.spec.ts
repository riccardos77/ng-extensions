...
describe('array.extension', () => {
  it('convertToObject - number array with string key', () => {
    ...
    const input = [1, 2, 3];
    const result = input.convertToObject(
      n => `k${n}`,
      n => n * 2
    );
  ...
  it('merge - custom', () => {
    ...
    const conflictFn = (i: MergeInput, o: MergeInput): MergeInput =>
      ({ id: i.id, value: o.value, otherProp: `${i.otherProp ?? ''}_${o.otherProp ?? ''}` } as MergeInput);
